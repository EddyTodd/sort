...
def read_rows(path: Path, candidates: set[str]) -> list[Observation]:
    required = {"algorithm", "n", "trial", "input_hash", "ns", "verified", "feature_ns", "sample_inversion_rate", "sample_duplicate_fraction", "sample_range_bits"}
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        for raw in reader:
            ...
            rows.append(Observation(algorithm=raw["algorithm"], n=int(raw["n"]), trial=int(raw["trial"]), input_hash=raw["input_hash"], ns=float(raw["ns"]), feature_ns=float(raw["feature_ns"]), inversion_rate=float(raw["sample_inversion_rate"]), duplicate_fraction=float(raw["sample_duplicate_fraction"]), range_bits=float(raw["sample_range_bits"])))
    return rows

def evaluate(rows: list[Observation], algorithms: list[str]) -> dict[str, object]:
    by_instance: dict[tuple[int, int, str], dict[str, Observation]] = defaultdict(dict)
    for row in rows:
        by_instance[(row.n, row.trial, row.input_hash)][row.algorithm] = row
    ...
    models = {algorithm: fit([group[algorithm] for group in training]) for algorithm in algorithms}
    training_medians = {algorithm: statistics.median(group[algorithm].ns for group in training) for algorithm in algorithms}
    ...
    return {"training_instances": len(training), "heldout_instances": len(heldout), "best_single": best_single, "heldout_best_single_median_ns": single, "heldout_portfolio_median_ns_including_features": selector, "portfolio_speedup_vs_best_single": single / selector if selector > 0 else math.nan, "heldout_oracle_median_ns": oracle, "portfolio_slowdown_vs_oracle": selector / oracle if oracle > 0 else math.nan, "selection_counts": dict(sorted(choices.items()))}
    ...
    for trial in range(12):
        for n in (16, 1024):
            for algorithm in ("a", "b"):
                ...
                rows.append(Observation(algorithm, n, trial, f"{n}-{trial}", ns, 1, 0.5, 0, 32))
...
def main() -> int:
    ...
    result = evaluate(read_rows(args.input, set(algorithms)), algorithms)
