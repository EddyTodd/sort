...
| adaptive merge stability | adaptive merge-policy track currently operates on scalar `int64_t` values | stability is established by construction but not yet revalidated with ordinal-carrying records in this track | add record/payload adaptive merge harness and empirical equal-key stability checks | open |
| adaptive merge kernel | common merge kernel lacks galloping and smaller-run temporary-buffer specialization | wall-time results isolate scheduling but are not a complete production TimSort/CPython comparison | add separately named galloping/buffer treatments; do not overwrite policy-only results | open |
