...
inline std::uint64_t bounded(std::mt19937_64& rng, std::uint64_t bound) {
...
inline std::uint64_t trial_seed(std::uint64_t seed, std::string_view pattern,
...
inline std::uint64_t input_hash(const std::vector<Value>& values) {
  std::uint64_t hash = 14695981039346656037ULL;
...
inline std::vector<Value> base_random(std::size_t n, std::uint64_t seed) {
  std::mt19937_64 rng(seed);
  std::vector<Value> out(n);
  for (auto& value : out) value = static_cast<Value>(bounded(rng, 2000000001ULL)) - 1000000000LL;
  return out;
}

inline std::vector<Value> make_data(std::string_view pattern, std::size_t n, std::uint64_t seed) {
  std::mt19937_64 rng(seed);
  auto a = base_random(n, seed);
  if (pattern == "random") return a;
  if (pattern == "sorted") {
  ...
  if (pattern == "binary") {
    for (auto& value : a) value = static_cast<Value>(bounded(rng, 2));
    return a;
  }
  ...
  if (pattern == "organ_pipe") {
    for (std::size_t i = 0; i < n; ++i) a[i] = static_cast<Value>(std::min(i, n == 0 ? 0 : n - 1 - i));
    return a;
  }
  if (pattern == "sawtooth") {
  ...
  if (pattern == "runs" || pattern == "descending_runs") {
    ...
    for (std::size_t base = 0; base < n; base += run) {
      ...
      if (pattern == "runs") {
        ...
        std::sort(a.begin() + static_cast<std::ptrdiff_t>(base),
                  a.begin() + static_cast<std::ptrdiff_t>(end), std::greater<>());
      }
    }
    return a;
  }
  if (pattern == "rotated") {
    std::sort(a.begin(), a.end());
    if (n > 1) std::rotate(a.begin(), a.begin() + static_cast<std::ptrdiff_t>(n / 3), a.end());
    return a;
  }
  if (pattern == "alternating_extremes") {
    Value low = std::numeric_limits<Value>::min();
    Value high = std::numeric_limits<Value>::max();
    for (std::size_t i = 0; i < n; ++i) a[i] = (i % 2 == 0) ? low++ : high--;
    return a;
  }
  if (pattern == "staggered") {
    constexpr std::size_t modulus = 97;
    for (std::size_t i = 0; i < n; ++i) a[i] = static_cast<Value>((i * 37) % modulus);
    return a;
  }
  if (pattern == "plateau") {
    for (std::size_t i = 0; i < n; ++i) {
      const auto distance = std::min(i, n == 0 ? 0 : n - 1 - i);
      a[i] = static_cast<Value>(std::min<std::size_t>(distance, 16));
    }
    return a;
  }
...
inline const std::vector<std::string>& all_patterns() {
...
inline bool verify(const std::vector<Value>& input, const std::vector<Value>& output) {
  auto expected = input;
...
inline std::vector<std::size_t> parse_sizes(std::string_view text) {
  std::vector<std::size_t> out;
  for (const auto& token : split_csv(text)) out.push_back(static_cast<std::size_t>(std::stoull(token)));
  return out;
}

inline std::vector<std::string> selected_patterns(const std::vector<std::string>& names) {
...
inline void deterministic_shuffle(std::vector<std::size_t>& values, std::uint64_t seed) {
  std::mt19937_64 rng(seed);
