...
constexpr void binary_insertion_impl(I first, I last, Ops& ops) {
    ...
    for (I it = first + 1; it < last; ++it) {
        ...
        while (lo < hi) {
            I mid = lo + (hi - lo) / 2;
            if (ops.less(value, *mid)) {
                hi = mid;
            } else {
                lo = mid + 1;
            }
        }
        for (I pos = it; pos > lo; --pos) {
            ops.assign(pos, std::ranges::iter_move(pos - 1));
        }
...
constexpr void selection_impl(I first, I last, Ops& ops) {
    for (I it = first; it < last; ++it) {
        I min_it = it;
        for (I scan = it + 1; scan < last; ++scan) {
            if (ops.less(*scan, *min_it)) {
                min_it = scan;
            }
        }
        ops.swap(it, min_it);
...
template <std::random_access_iterator I, class Ops>
constexpr void shell_with_gaps(I first, I last, Ops& ops,
...
constexpr void sift_down(I first, std::size_t root, std::size_t n, Ops& ops) {
    for (;;) {
        ...
        if (child + 1 < n &&
            ops.less(*(first + static_cast<std::iter_difference_t<I>>(child)),
                     *(first + static_cast<std::iter_difference_t<I>>(child + 1)))) {
            ++child;
        }
